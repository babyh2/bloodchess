...
int main(int argc, char *argv[])
{
    ...
    MainObject p_player;
    // Preload player sprites once to avoid reloading images each frame
    ...
    while (currentState != EXIT)
    {
        ...
        case GAME:
        {
            ...
            // Clear screen then render background
...
bool InitData()
{
    ...
    else
    {
        ...
        else
        {
            ...
            if ((IMG_Init(imgFlg) & imgFlg) != imgFlg)
            {
                success = false;
            }
        }
...
void RenderTextFor5Seconds(SDL_Renderer *screen, TTF_Font *font, const std::string &text, SDL_Color color)
{
    ...
    // We'll render it for up to 5 seconds but keep polling events so the window stays responsive
    ...
    while (running)
    {
        ...
        if (SDL_GetTicks() - start >= 5000)
            running = false;
        SDL_Delay(16); // small sleep to avoid busy loop (~60 FPS)
    }
...
void close()
{
    ...
    // Shutdown SDL subsystems in proper order
    TTF_Quit();
    IMG_Quit();
    SDL_Quit();
}
