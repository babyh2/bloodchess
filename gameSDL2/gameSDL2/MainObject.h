...
public:
	...
	// Load all player direction sprites once (avoid loading from disk each frame)
	bool LoadPlayerImages(SDL_Renderer *screen);
...
private:
	...
	// cache sprites for different directions to avoid reloading every frame
	BaseObject sprite_right_;
	BaseObject sprite_left_;
	BaseObject sprite_up_;
	BaseObject sprite_down_;
	BaseObject sprite_alt_; // optional alternate character (joker)
};
